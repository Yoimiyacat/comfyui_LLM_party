...
## Загрузка
* Язык можно настроить в `config.ini`, в настоящее время доступны только китайский (zh_CN) и английский (en_US), по умолчанию используется язык вашей системы.
...
* В `config.ini` в секции `[API_KEYS]` вы можете задать `default_api_model` (например `gpt-4o-mini`). Когда имя модели в загрузчике API LLM установлено на `default`, используется эта модель. Это позволяет переключать модель API по умолчанию в одном месте без изменения рабочих процессов.
...
6. Каждая модель может быть настроена отдельно в файле `config.ini`, вы можете обратиться к файлу `config.ini.example` для справки. После настройки вам нужно просто ввести `model_name` на узле. Вы также можете установить `model_name` в `default`, чтобы использовать `default_api_model` из `config.ini`.
